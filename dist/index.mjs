...
import ECB from "crypto-js/mode-cfb.js";
...
const aesEncrypt = (word, encryptKey) => {
    const key = Utf8.parse(encryptKey);
    const wordArray = Utf8.parse(word);
...
const aesDecrypt = (word, decryptKey) => {
    const key = Utf8.parse(decryptKey);
    const bytes = AES.decrypt(word, key, {
        mode: ECB,
        padding: Pkcs7,
    });
    const originalText = Utf8.stringify(bytes).toString();
...
//#endregion
export {
    Checker_default as Checker,
    Crypto_default as Crypto,
    Date_default as DateX,
    Download_default as Download,
    Enum_default as EnumX,
    Is_default as Is,
    LocalStorage_default as LocalStorage,
    Number_default as NumberX,
    String_default as StringX,
};
